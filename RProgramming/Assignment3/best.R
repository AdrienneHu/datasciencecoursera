...
#identifier function for getting the names of the hospitals
identifier <- function(data, col_num, state){
  ...
  # change data type from character to numeric
    ...
    if(!state %in% data$State){
        ...
        if(outcome == "heart attack") {
            hosp_name <- identifier(data, 11,state)
        } else if(outcome =="heart failure"){
            hosp_name <- identifier(data, 17,state)
        } else{
            hosp_name <- identifier(data, 23,state)
        }
        result <-hosp_name
        return(result)
    }
